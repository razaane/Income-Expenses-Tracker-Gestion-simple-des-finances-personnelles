...
for($month = 1; $month <= 12; $month++) {
  ...
  // Revenus du mois
  $stmt_inc = $pdo->query("SELECT COALESCE(SUM(montant), 0) AS total FROM incomes WHERE YEAR(la_date) = $current_year AND MONTH(la_date) = $month");
  $month_income = $stmt_inc->fetch(PDO::FETCH_ASSOC)['total'];
  
  // Dépenses du mois
  $stmt_exp = $pdo->query("SELECT COALESCE(SUM(montant), 0) AS total FROM expenses WHERE YEAR(la_date) = $current_year AND MONTH(la_date) = $month");
  ...
  <div class="flex min-h-screen">
    ...
    <div id="mobile-sidebar" class="hidden fixed inset-0 z-50 lg:hidden">
      ...
      <aside class="absolute left-0 top-0 h-full w-64 bg-[color:var(--card-bg)] border-r border-gray-200 dark:border-gray-800 flex flex-col">
        ...
        <!-- Navigation -->
        <nav class="flex-1 p-4 space-y-2">
          <a href="index.php" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-[color:var(--accent-light)] text-white transition-all">
            <span class="material-symbols-outlined">dashboard</span>
            <span class="font-medium">Dashboard</span>
          </a>
    ...
    <div id="modal-expense" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-lg rounded-2xl bg-[color:var(--card-bg)] p-6 shadow-lg border border-gray-100 dark:border-gray-800">
        ...
        <form method="POST" action="traitement.php" class="grid grid-cols-1 gap-3">
          ...
          <div>
            <label class="text-sm text-gray-600 dark:text-[color:var(--muted)]">Description</label>
            <textarea name="descreption_exp" class="h-20 w-full rounded-xl border border-gray-200 px-3 py-2 focus-ring" required></textarea>
          </div>
          <button type="submit" name="save_exp" class="px-4 py-2 rounded-xl text-white" style="background: linear-gradient(90deg,var(--accent-light),#4f46e5);">Save</button>
        </form>
      </div>
    </div>
